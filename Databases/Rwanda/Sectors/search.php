...
<body>
    ...
    if (!$connection) {
        ...
        $sql = "SELECT * FROM sectors where sectorId like '%$id%'";
        ...
        $sql = "SELECT * FROM sectors where sectorName like '%$name%'";
        $select = mysqli_query($connection, $sql) or die("Error occurred" . mysqli_error($connection));
        $result = $connection->query($sql);
        if ($result->num_rows > 0) {
            ...
            echo "</table>";
            echo "<a href='display.php'>Display all sectors</a>";
        } else {
            echo "No sector has a name of " . $name . "";
            echo "<a href='display.php'>Go back</a>";
        }
        $connection->close();
    }
